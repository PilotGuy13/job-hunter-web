...
def dashboard():
    ...
    # ── Metrics ──────────────────────────────────────────────────────────────
    base = JobResult.query.filter_by(user_id=current_user.id, dismissed=False)

    total_jobs     = base.count()
    high_match     = base.filter(JobResult.compatibility_score >= 80).count()
    saved_count    = base.filter_by(saved=True).count()
    apps_count     = base.filter(JobResult.app_status.notin_(["watching"])).count()
    avg_score      = db.session.query(func.avg(JobResult.compatibility_score)).filter(
                        JobResult.user_id == current_user.id,
                        JobResult.dismissed == False
                     ).scalar() or 0

    # Top 5 matching jobs
    top_jobs = base.order_by(JobResult.compatibility_score.desc()).limit(5).all()

    # Application pipeline counts
    pipeline = {}
    for status_id in ["watching", "applied", "interviewing", "offered", "rejected", "withdrawn"]:
        pipeline[status_id] = base.filter_by(app_status=status_id).count()

    # Upcoming interviews (next 30 days)
    from datetime import timedelta
    upcoming_interviews = (base
        .filter(JobResult.interview_date >= datetime.utcnow())
        .filter(JobResult.interview_date <= datetime.utcnow() + timedelta(days=30))
        .order_by(JobResult.interview_date.asc())
        .limit(5).all())

    # Jobs per source (search coverage)
    source_counts = (db.session.query(JobResult.source, func.count(JobResult.id))
                     .filter_by(user_id=current_user.id, dismissed=False)
                     .group_by(JobResult.source)
    ...
    return render_template("dashboard.html",
        total_jobs=total_jobs, high_match=high_match, saved_count=saved_count,
        apps_count=apps_count, avg_score=round(avg_score), top_jobs=top_jobs,
...
@login_required
def job_calendar(job_id):
    """Generate .ics calendar event for a job interview."""
    job = JobResult.query.filter_by(id=job_id, user_id=current_user.id).first_or_404()
    if not job.interview_date:
        return "No interview date set", 400

    from datetime import timedelta
    start = job.interview_date
    end = start + timedelta(hours=1)

    ics = f"""BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//JobHunterApp.io//Interview//EN
BEGIN:VEVENT
...
END:VCALENDAR"""
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment;filename=interview_{job.company.replace(' ','_')}.ics"}
    )
